...
begin.lines <-
	data.frame(
		year = 2013:2005 ,
		household = c( 993 , 993 , 988 , 953 , 994 , 981 , 994 , 989 , 992 ) ,
		family = c( 1129 , 1129 , 1121 , 1082 , 1137 , 1124 , 1143 , 1138 , 1141 ) ,
		person = c( 1218 , 1218 , 1209 , 1166 , 1221 , 1208 , 1227 , 1222 , 1225 )
	)
...
for ( year in cps.years.to.download ){
	...
	CPS.ASEC.mar.file.location <- 
		ifelse( 
			...
			ifelse(
				year == 2013 ,
				"http://smpbff2.dsd.census.gov/pub/cps/march/asec2013early_pubuse.zip" ,
