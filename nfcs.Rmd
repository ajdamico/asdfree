...
* One state-by-state survey table with one row per sampled respondent, separate investor survey.
...
1. [2021 National Financial Capability Study: State-by-State Survey Methodology](https://finrafoundation.org/sites/finrafoundation/files/NFCS-2021-State-by-State-Methodology.pdf)

2. [Financial Capability Insights: What the NFCS Reveals](https://www.finrafoundation.org/knowledge-we-gain-share/nfcs/other-research)

3. This human-composed haiku or a bouquet of [artificial intelligence-generated limericks](https://www.gnod.com/search/ai#q=write%20a%20limerick-style%20poem%20about%20the%20National Financial Capability Study)
...
This example matches the national estimate of _53% have three months of rainy day funds_ from
[PDF page 7](https://finrafoundation.org/sites/finrafoundation/files/NFCS-Report-Fifth-Edition-July-2022.pdf#page=7):
```{r eval = FALSE , results = "hide" }
national_rainy_day <- svymean( ~ rainy_day_fund , nfcs_design )
stopifnot( round( coef( national_rainy_day )[ 'rainy_day_fundYes' ] , 2 ) == 0.53 )
```

This example matches the state-based unweighted counts and rainy day estimates from [The Geography of Financial Capability](https://cdn.finra.org/nfcs/2021/geography.html):
```{r eval = FALSE , results = "hide" }
state_counts <-
	svyby(
		~ one ,
