...
###############################################################################################
# Download All 2008 Panel Core Files of the Survey of Income and Program Participation with R #
###############################################################################################
...
# choose which core file waves to download
# recommended download: all available
sipp.datasets.to.download <- 1:10		# download waves 1 through 10

###############################################
# DATA LOADING COMPONENT - ONLY RUN THIS ONCE #
###############################################

# each of the core file waves uses the same SAS input script
...
for ( i in sipp.datasets.to.download ){
	...
	# "SIPP.08w#.df" -- where # is the current wave
	assign(	df.name , x )

	# save the data frame as an external file in the current working directory
	save( list = df.name , file = paste0( "SIPP.08w" , i , ".rda" ) )
