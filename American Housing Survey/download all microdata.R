...
for ( curdir in rev( precise.links ) ){
	...
	# extract the current year of data (that is, extract all numbers from the string)
	this.year <- as.numeric( gsub( "\\D" , "" , curdir ) )
	
	# create a character string with the current working directory plus the current year
	year.dir <- paste0( getwd() , "/" , this.year )
	
	# create that directory on the local disk
	dir.create( year.dir )

	# read in all specific files within this folder
	# by scraping the ahs ftp web page contents into a character vector
	tf <- getURL( paste0( ahs.ftp , curdir , '/' ) )
	...
	sas.matches <- pfi[ duplicated( pfi ) ]
	
	# get rid of the sas version if there's a csv version.
	precise.files <- precise.files[ !( precise.files %in% sas.matches ) ]
	
	# do not download flat files, since they're only in sas7bdat and so hard to import.
	precise.files <- precise.files[ !grepl( 'Flat' , precise.files ) ]
	...
	# hard-code the removal of files that don't make sense.
	if ( curdir == 'AHS_2009' ) precise.files <- precise.files[ precise.files != 'AHS_2011_Topcodes.zip' ]
	...
	for ( fn in precise.files ){
		...
		# construct the full ftp filepath of the file to-be-downloaded
		full.fn <- paste0( ahs.ftp , curdir , '/' , fn )
		
		# initiate a new temporary file on the local disk
		tf <- tempfile() 
		
		# download the exact file to the local disk
		download.cache( full.fn , tf , mode = 'wb' )

		# extract the filename (without extension)
		prefix <- gsub( "\\.(.*)" , "" , basename( full.fn ) )
		...
		if ( extension %in% 'zip' ){
			...
			if( length( tf ) == 0 ){
				...
				download.cache( full.fn , tf , mode = 'wb' , usecache = FALSE )
				...
				paste0(
					...
					gsub( 
						...
						substr( 
							basename( full.fn ) ,
							1 ,
							nchar( basename( full.fn ) ) - 4
						)
					)
				)
			
			# 2001 has a backward filename that's causing a duplicate to be missed
			zip.path <- gsub( paste0( this.year , "_AHS" ) , "" , zip.path )
			...
			zip.path <- gsub( "_CSV" , "" , zip.path )
			zip.path <- gsub( "_PUF" , "" , zip.path )
			zip.path <- gsub( "PUF_" , "" , zip.path )
		...
		for ( i in seq( extension ) ){
			...
			if( extension[ i ] %in% c( 'xpt' , 'sas7bdat' , 'csv' ) ){

				# ..try to import it using the appropriate function
				import.attempt <-
					try( {
					
							if ( extension[ i ] == 'xpt' ) x <- read.xport( tf[ i ] )
				...
				if ( class( import.attempt ) != 'try-error' ){
					...
					dir.create( paste0( year.dir , zip.path ) , showWarnings = FALSE )
