...
Download and import the latest quarterly file:
...
sas_results[ , 'character' ] <- grepl( '\$' , sas_results[ , 'length' ] )

sas_results[ , 'position' ] <- as.integer( gsub( "\@" , "" , sas_results[ , 'position' ] ) )

sas_results[ , 'length' ] <- as.integer( gsub( "\$" , "" , sas_results[ , 'length' ] ) )

stopifnot( sum( sas_results[ , 'length' ] ) == ( sas_results[ nrow( sas_results ) , 'position' ] + sas_results[ nrow( sas_results ) , 'length' ] - 1 ) )
...
this_url <-
	"https://ftp.ibge.gov.br/Trabalho_e_Rendimento/Pesquisa_Nacional_por_Amostra_de_Domicilios_continua/Trimestral/Microdados/2023/PNADC_012023.zip"
...
pnadc_tbl <-
	readr::read_fwf(
			this_tf ,
			fwf_widths( 
				widths = sas_results[ , 'length' ] , 
				col_names = sas_results[ , 'column_name' ] 
			) ,
			col_types = 
				paste0( ifelse( sas_results[ , 'character' ] , "c" , "d" ) , collapse = '' )
		)
