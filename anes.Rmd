...
The American National Election Studies (ANES) collects information on political belief and behavior from eligible voters in the United States.
...
1. [ANES 2020 Time Series Study Full Release: User Guide and Codebook](https://electionstudies.org/wp-content/uploads/2022/02/anes_timeseries_2020_userguidecodebook_20220210.pdf)

2. [Wikipedia Entry](https://en.wikipedia.org/wiki/European_Social_Survey)

3. This human-composed haiku or a bouquet of [artificial intelligence-generated limericks](https://www.gnod.com/search/ai#q=write%20a%20limerick-style%20poem%20about%20the%20American National Election Studies)
...
<center>
<img src="http://4.bp.blogspot.com/-uAZOgrbmKFY/UJM93hPjaoI/AAAAAAAAYF8/q3-VM4qAzF4/s1600/IMG_2268.JPG">
<p>
<i><b>the author as speaker boehner</b></i>
</center>
...
This example matches statistics and confidence intervals in the Age rows of the `ANES respondents (weighted)` column of `Table 1A` from [Guide to Using Weights and Sample Design Indicators with ESS Data](https://electionstudies.org/wp-content/uploads/2022/03/DeBell-Maisel-Benchmark-Report-for-the-ANES-2016-Time-Series-Study.pdf#page=4):

1. Choose `2016 Time Series Study`.

2. Download the `DTA` version of the September 4, 2019 file

3. Download the `DTA` version of the `Methodology File December 10, 2018`
...
stopifnot( all( round( coef( results ) , 3 ) == c( 0.21 , 0.158 , 0.156 , 0.2 , 0.147 , 0.129 ) ) )

stopifnot( all( round( SE( results ) , 4 ) == c( 0.0091 , 0.009 , 0.0094 , 0.0122 , 0.0069 , 0.0083 ) ) )
```

This example matches statistics and confidence intervals in the Age rows of the `Design-consistent, with published strata` column of `Table 1` from [How to Analyze ANES Survey Data](https://electionstudies.org/wp-content/uploads/2018/04/nes012492.pdf#page=25):

1. Choose `2004 Time Series Study`4

2. Download the `DTA` version of the Full Release August 16, 2005 file

3. Choose `2006 Pilot Study`

4. Download the `DTA` version of the April 26, 2007 file
...
stopifnot( all( round( coef( results ) , 3 ) == c( 0.207 , 0.162 , 0.218 , 0.175 , 0.111 , 0.126 ) ) )

stopifnot( all( round( SE( results ) , 4 ) == c( 0.0251 , 0.024 , 0.022 , 0.0149 , 0.0125 , 0.0287 ) ) )
```
