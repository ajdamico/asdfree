...
ncvs_badge <- paste0( "<img src='https://img.shields.io/badge/tested%20on%20my%20laptop:-" , most_recent_build_date , "-brightgreen' alt='Local Testing Badge'>" )
...
* Three main tables, the first with one row per sampled household per interview, the second with one row per person per interview, and the third with one row per incident reported across each sampled household's seven-interview, three-year period.

* A complex survey designed to generalize to civilian, non-institutional americans aged 12 and older.

* Released annually since its re-design (and rename) in 1992, the National Crime Survey dates back to the early 1970s.
...
3. This human-composed haiku or a bouquet of [artificial intelligence-generated limericks](https://www.gnod.com/search/ai#q=write%20a%20limerick-style%20poem%20about%20the%20National Crime Victimization Survey)
...
left_join_zero_missings <-
	function( left_df , right_df ){
		...
		for( this_column in setdiff( names( right_df ) , names( left_df ) ) ) final_df[ is.na( final_df[ , this_column ] ) , this_column ] <- 0
...
ncvs_household_df <-
	ncvs_household_df[ 
		ncvs_household_df[ , 'year' ] %in% setdiff( 1994:2022 , 2006 ) , 
		household_variables_to_keep
	]

ncvs_df <-
	ncvs_df[ 
		ncvs_df[ , 'year' ] %in% setdiff( 1994:2022 , 2006 ) , 
		person_variables_to_keep
	]

ncvs_incident_df <-
	ncvs_incident_df[
		ncvs_incident_df[ , 'year' ] %in% setdiff( 1994:2022 , 2006 ) , 
		incident_variables_to_keep
	]
```
...
ncvs_df[ c( 'idhh' , 'idper' ) ] <-
	sapply( ncvs_df[ c( 'idhh' , 'idper' ) ] , as.character )
...
ncvs_df[ sapply( ncvs_df , class ) == 'factor' ] <-
	sapply( 
		ncvs_df[ sapply( ncvs_df , class ) == 'factor' ] , 
		ncvs_numeric_to_factor ,
		simplify = FALSE
...
ncvs_df[ , 'yr_grp' ] <-
	findInterval( ncvs_df[ , 'year' ] , c( 1992 , 1997 , 2006 , 2016 ) )
...
names( summed_hh_crimes ) <-
	c( 'yearq' , 'idhh' , 'v2002' , 'wgtviccy' , 'property' , 'hhburg' , 'mvtft' , 'burg' , 'tres' )
```
...
names( summed_person_crimes ) <-
	c( 'yearq' , 'idhh' , 'v3002' , 'idper' , 'wgtviccy' , 'violent' , 'sviolent' , 'rsa' , 'rob' , 'aast' , 'sast' , 'ptft' )
```

Merge aggregated property-crimes on to the person-interview data.frame:
```{r cache = TRUE , warning = FALSE , message = FALSE , results = "hide" }
ncvs_df <- left_join_zero_missings( ncvs_df , summed_person_crimes )
...
names( summed_person_year_violent_crimes )[ ncol( summed_person_year_violent_crimes ) ] <- 'violent_year'
```

Merge aggregated person-year violent crime series weights on to the person-interview data.frame:
```{r cache = TRUE , warning = FALSE , message = FALSE , results = "hide" }
ncvs_df <- left_join_zero_missings( ncvs_df , summed_person_year_violent_crimes )
...
```{r cache = TRUE , warning = FALSE , message = FALSE , results = "hide" }
ncvs_df <- left_join_zero_missings( ncvs_df , wide_person_half_year_violent_crimes )
...
maximum_half_violent_crimes <-
	maximum_half_violent_crimes[ do.call( order , maximum_half_violent_crimes[ c( 'idhh' , 'idper' , 'year' , 'half_year' ) ] ) , ]

maximum_half_violent_crimes[ , 'byvar' ] <-
	apply( maximum_half_violent_crimes[ c( 'idhh' , 'idper' , 'year' , 'half_year' ) ] , 1 , paste , collapse = ' ' )
	
maximum_half_violent_crimes[ 1 , 'id' ] <- 1

for( i in seq( 2 , nrow( maximum_half_violent_crimes ) ) ){

	if( maximum_half_violent_crimes[ i , 'byvar' ] == maximum_half_violent_crimes[ i - 1 , 'byvar' ] ){
	
		maximum_half_violent_crimes[ i , 'id' ] <- maximum_half_violent_crimes[ i - 1 , 'id' ] + 1
		
	} else {
	
		maximum_half_violent_crimes[ i , 'id' ] <- 1
		
	}
}

maximum_half_violent_crimes[ , 'label' ] <- paste0( '_' , maximum_half_violent_crimes[ , 'half_year' ] , '_' , maximum_half_violent_crimes[ , 'id' ] )

maximum_half_violent_crimes[ , 'byvar' ] <- NULL

stopifnot( all( maximum_half_violent_crimes[ , 'label' ] %in% c( '_1_1' , '_2_1' , '_1_2' ) ) )

h_1_1_df <-
	maximum_half_violent_crimes[ 
		maximum_half_violent_crimes[ , 'label' ] == '_1_1' , 
...
h_2_1_df <-
	maximum_half_violent_crimes[ 
		maximum_half_violent_crimes[ , 'label' ] == '_2_1' , 
		c( 'idhh' , 'idper' , 'year' , 'wgtviccy' )
	]
...
h_1_2_df <-
	maximum_half_violent_crimes[ 
		maximum_half_violent_crimes[ , 'label' ] == '_1_2' , 
		c( 'idhh' , 'idper' , 'year' , 'wgtviccy' )
	]
...
```{r cache = TRUE , warning = FALSE , message = FALSE , results = "hide" }
ncvs_df <- left_join_zero_missings( ncvs_df , three_half_df )
...
summed_person_year_interviews <-
	aggregate(
		one ~ idhh + idper + year ,
		data = subset( ncvs_df , wgtpercy > 0 ) ,
		sum
	)

names( summed_person_year_interviews )[ ncol( summed_person_year_interviews ) ] <- 'interview_count'
```

Merge interview_count on to the person-interview data.frame:
```{r cache = TRUE , warning = FALSE , message = FALSE , results = "hide" }
ncvs_df <- left_join_zero_missings( ncvs_df , summed_person_year_interviews )
...
ncvs_df <-
	transform(
		...
		interview_incident_groups =
			ifelse( violent_year == 0 , 1 ,
			ifelse( interview_count == 1 & ( ( as.numeric( vwgt1 > 0 ) + as.numeric( vwgt2 > 0 ) ) == 1 ) & wgtviccy > 0 , 2 ,
			ifelse( interview_count == 2 & ( ( as.numeric( vwgt1 > 0 ) + as.numeric( vwgt2 > 0 ) ) == 1 ) , 3 ,
			ifelse( interview_count == 2 & ( vwgt1 > 0 ) & ( vwgt2 > 0 ) & ( wgtviccy > 0 ) , 4 ,
			ifelse( interview_count == 3 & ( ( as.numeric( wgtviccy_1_1 > 0 ) + as.numeric( wgtviccy_2_1 > 0 ) + as.numeric( wgtviccy_1_2 > 0 ) ) == 1 ) , 5 ,
			ifelse( interview_count == 3 & ( wgtviccy_1_1 > 0 ) & ( wgtviccy_2_1 > 0 ) & ( wgtviccy_1_2 > 0 ) , 6 ,
			ifelse( interview_count == 3 & ( wgtviccy_1_1 > 0 ) & ( wgtviccy_2_1 > 0 ) & substr( yearq , 6 , 6 ) %in% 1:2 , 7 ,
			ifelse( interview_count == 3 & ( wgtviccy_1_1 > 0 ) & ( wgtviccy_2_1 > 0 ) & substr( yearq , 6 , 6 ) %in% 3:4 , 8 , 9 ) ) ) ) ) ) ) )
	)
			

# confirm all records in group 9 have both a wgtviccy == 0 & wgtpercy == 0
stopifnot( nrow( subset( ncvs_df , interview_incident_groups == 9 & wgtviccy > 0 ) ) == 0 )
stopifnot( nrow( subset( ncvs_df , interview_incident_groups == 9 & wgtpercy > 0 ) ) == 0 )
...
ncvs_df <-
	transform(
		...
		prev_wgt0 =
			...
			ifelse( interview_incident_groups == 5 , pmax( wgtviccy_1_1 , wgtviccy_1_2 , wgtviccy_2_1 , na.rm = TRUE ) / 2 ,
...
summed_year_weights <-
	aggregate(
		...
		data = subset( ncvs_df , violent_year == 1 ) ,
		sum
	)

names( summed_year_weights ) <- c( 'year' , 'vwgt_1v' , 'prev_1v' )

ncvs_df <- left_join_zero_missings( ncvs_df , summed_year_weights )
...
ncvs_df <- left_join_zero_missings( ncvs_df , summed_year_crimes )
...
ncvs_df[ is.na( ncvs_df[ , 'v2118' ] ) , 'v2118' ] <- -1
ncvs_df[ is.na( ncvs_df[ , 'v2117' ] ) , 'v2117' ] <- -1
	...
	update( 
		...
		current_marital_status =
			factor( 
				v3015 , 
				levels = c( 1:5 , 8 ) , 
				labels = c( 'married' , 'widowed' , 'divorced' , 'separated' , 'single' , 'residue' ) ) ,
		
		household_income_starting_2015q1 =
			factor(
				findInterval( sc214a , c( 1 , 9 , 13 , 16 , 18 ) ) ,
				levels = 1:5 ,
				labels = c( 'less than $25,000' , '$25,000 - $49,999' , '$50,000 - $99,999' , '$100,000 - $199,999' , '$200,000 or more' )
			) ,
...
stopifnot( 
	round( coef( new_prevalence_results )[ c( 1 , nrow( new_prevalence_results ) ) ] , 4 ) == c( 0.0442 , 0.0151 )
)

# match new method (wgt_ovam) 1994 and 2022 standard errors
stopifnot( 
	round( SE( new_prevalence_results )[ c( 1 , nrow( new_prevalence_results ) ) ] , 5 ) == c( 0.0010 , 0.00054 )
)
...
stopifnot( 
	round( coef( old_prevalence_results )[ c( 1 , nrow( old_prevalence_results ) ) ] , 4 ) == c( 0.0328 , 0.0124 )
)

# match old method (wgtpercy) 1994 and 2022 standard errors
stopifnot( 
	round( SE( old_prevalence_results )[ c( 1 , nrow( old_prevalence_results ) ) ] , 5 ) == c( 0.00075 , 0.00042 )
)
```
